...
const kategoriKerugian = [
  'Produk rusak', 'Paket hilang', 'Return', 'Refund', 'Lainnya'
];
...
export default function Kerugian() {
  const [searchTerm, setSearchTerm] = useState('');
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const queryClient = useQueryClient();

  const { data: kerugian = [], isLoading } = useQuery({
    queryKey: ['kerugian'],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const { error } = await supabase.from('kerugian').insert(data);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['kerugian'] });
      toast.success('Data kerugian berhasil ditambahkan');
    },
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('kerugian').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['kerugian'] });
  ...
  const filteredData = kerugian.filter(item => 
    (item.kategori || '').toLowerCase().includes(searchTerm.toLowerCase())
  );

  const thisMonthData = kerugian.filter(item => {
  ...
  const columns = [
    ...
    { header: 'Kategori', accessor: 'kategori' as keyof typeof kerugian[0] },
    { header: 'Nominal', accessor: (row: any) => formatCurrency(row.nominal), className: 'text-destructive font-semibold' },
    { header: 'Keterangan', accessor: 'keterangan' as keyof typeof kerugian[0] },
    {
      header: 'Aksi',
      accessor: (row: any) => (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Kerugian</h1>
          <p className="text-muted-foreground mt-2">Monitor kerugian bisnis</p>
        </div>
        <ExportButtons />
      </div>
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-center gap-3">
            ...
            <div className="text-center">
              <p className="text-sm text-muted-foreground">Total Kerugian Bulan Ini</p>
              <p className="text-3xl font-bold text-destructive mt-1">{formatCurrency(totalNominal)}</p>
      ...
      <Card className="shadow-card">
        <CardHeader>
          <CardTitle>Data Kerugian</CardTitle>
        </CardHeader>
        <CardContent>
